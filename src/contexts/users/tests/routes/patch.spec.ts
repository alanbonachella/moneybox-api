...
describe("PATCH /users/:userId", () => {
  ...
  describe("successes", () => {
    test(`Should update a user's name only`, async () => {
      ...
      expect(user.cpf).toBe(persistedData.cpf.trim());
    ...
    test(`Should update a user's CPF only`, async () => {
      const persistedData: User = usersFixture();
      const data = {
        name: faker.helpers.replaceSymbolWithNumber("###.###.###-##"),
      };
    ...
    test(`Should update a user's enabled only`, async () => {
      const persistedData: User = usersFixture();
      const data = {
        enabled: !persistedData.enabled,
      };
  ...
  describe("failures", () => {
    test("Should not update a user with unauthorized user", async () => {
      ...
      const res = await _request.patch(`/users/${persistedData.id}`).send(data);
    ...
    test("Should not update a user with an unknown user ID data", async () => {
      const data: UserRequest = usersRequestFixture();
      const persistedData: User = usersFixture();
      const anotherUser: User = usersFixture();
